...
const { returnRolesList, getRoleID } = require("../lib/Role");
...
class User {
  ...
  addEmployee(values) {
    ...
    let managerId = [];
    let roleID = [];
    getRoleID(values.newEmpRole).then((role) => {
      roleID.push(role[0].id);
    });
    getManagerID(values.emplsManager)
      ...
      .then(() => {
        ...
        VALUES (?, ?, ?, ?)`;
        const arg = [firstName, lastName, roleID, managerId];
  ...
  updateEmployee(values) {
    ...
    console.log("these are the args:", arg)
